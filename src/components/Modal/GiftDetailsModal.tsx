...
export const GiftDetailsModal = ({ data, onClose }: GiftDetailsModalProps) => {
  ...
  const handleSellChannel = async (_itemId: string, price: number, duration?: number) => {
  ...
  const handleChangePrice = async (_itemId: string, newPrice: number) => {
    ...
    try {
      ...
      showSuccessToast({ message: t('toast.channelPriceUpdatedSuccess') });
      onClose();
    } catch (error) {
      console.error('Failed to update channel price:', error);
      const errorMessage = (error as any)?.message || t('toast.channelPriceUpdateFailed');
      showErrorToast({ message: errorMessage });
    }
  };
  ...
  const handleReturnChannel = async () => {
    ...
    try {
      ...
      showSuccessToast({ message: t('toast.channelReturnedSuccess') });
      onClose();
    } catch (error) {
      console.error('Failed to return channel:', error);
      const errorMessage = (error as any)?.message || t('toast.channelReturnFailed');
      showErrorToast({ message: errorMessage });
    }
  };
  ...
  const handleRemoveChannel = async () => {
    ...
    try {
      ...
      showSuccessToast({ message: t('toast.channelRemovedSuccess') });
      onClose();
    } catch (error) {
      console.error('Failed to remove channel:', error);
      const errorMessage = (error as any)?.message || t('toast.channelRemoveFailed');
      showErrorToast({ message: errorMessage });
    }
  };
  ...
  return (
    <div className="market-header__sheet">
            ...
            return (
              <div className={`product-sheet__grid product-sheet__grid--${gridClass}`}>
                ...
                <div className="product-sheet__cell">
                    <div className="product-sheet__more-badge"><span>+{count - 3} {t('channel.moreItems')}</span></div>
                  </div>
                )}
              </div>
            );
              ...
              <div className="product-sheet__row-main">
                ...
                {!channel.my_channel ? (
                  <div className="product-sheet__row-note">
                    {t('channel.waitingForReceiving')}
                  </div>
                ) : (
                <div className="product-sheet__row-note">
                  {t('channel.waitingForTransfer')} {channel.transferring_to_full_name} <Link href={`https://t.me/${channel.transferring_to_username}`}>@{channel.transferring_to_username}</Link> [{channel.transferring_to_id}].
                </div>
                )}
              </div>
          </div>
        ...
        {items.map((it: any) => (
          <div key={it.id} className="product-sheet__row">
            ...
            <div className="product-sheet__row-main">
              ...
              <div className="product-sheet__row-note">{t('channel.quantity')}: {it.quantity}</div>
            </div>
            {it?.type === 'nft' && (
              <div>
                <NftBadge label={t('gift.nft')} />
              </div>
            )}
          </div>
        ))}
      ...
      <div style={{borderTop: '0'}} className="product-sheet__actions">
        ...
        <Link className="product-sheet__btn product-sheet__btn--primary" href={channel.invite_link}>
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="product-sheet__actions">
          {channel?.status === 'reserved' ? (
            <>
              ...
              >
                {channel.type === 'fast' 
                  ? (returnChannelMutation.isPending ? t('channel.returning') : t('channel.return'))
                  : (removeChannelMutation.isPending ? t('channel.removing') : t('channel.remove'))
                }
              </button>
              <button 
                className="product-sheet__btn product-sheet__btn--primary" 
              ...
              >
                {sellChannelMutation.isPending ? t('channel.listing') : t('channel.sellChannel')}
              </button>
            </>
            ...
            <>
              <button 
                ...
                onClick={async () => {
                  try {
                    ...
                    showSuccessToast({ message: t('toast.channelRemovedFromSaleSuccess') });
                    onClose();
                  } catch (error) {
                    console.error('Failed to remove channel from sale:', error);
                    const errorMessage = (error as any)?.message || t('toast.channelRemovedFromSaleFailed');
                    showErrorToast({ message: errorMessage });
                  }
                }}
              >
                {removeChannelFromSaleMutation.isPending ? t('channel.removing') : t('channel.removeSale')}
              </button>
              <button 
                className="product-sheet__btn product-sheet__btn--primary" 
                style={{display: 'inline-block'}} 
              ...
              >
                {editChannelPriceMutation.isPending ? t('channel.updating') : t('channel.changePrice')}
                <span className="product-sheet__price">{price} TON</span>
              </button>
            </>
          ) : (
            <>
              <button className="product-sheet__btn" type="button" onClick={handleMakeOffer}>{t('channel.makeOffer')}</button>
              <button className="product-sheet__btn product-sheet__btn--primary" style={{display: 'inline-block'}} type="button" onClick={handleBuyChannel}>{t('channel.buyChannel')}
                <span className="product-sheet__price">{price} TON</span>
              </button>
            </>
          )}
