...
import { copyTextToClipboard, initData, openLink } from "@telegram-apps/sdk-react";
...
export const SettingsModal: FC<SettingsModalProps> = ({ onClose }) => {
  ...
  const handleLanguageChange = async (language: 'EN' | 'RU' | 'ZH') => {
    setSelectedLanguage(language);
    const backendLang = getLanguageForBackend(language);
    try {
      await updateLanguageMutation.mutateAsync(backendLang);
      // Also update i18n language immediately for UI
      i18n.changeLanguage(backendLang);
      showSuccessToast({ message: t('settings.languageUpdated') });
    } catch (error) {
      console.error('Failed to update language:', error);
      const errorMessage = (error as any)?.message || t('settings.languageUpdateFailed');
      showErrorToast({ message: errorMessage });
    }
  };

  const handleThemeChange = async (theme: 'WHITE' | 'SYSTEM' | 'DARK') => {
    setSelectedTheme(theme);
    try {
      await updateThemeMutation.mutateAsync(getThemeForBackend(theme));
      showSuccessToast({ message: t('settings.themeUpdated') });
    } catch (error) {
      console.error('Failed to update theme:', error);
      const errorMessage = (error as any)?.message || t('settings.themeUpdateFailed');
      showErrorToast({ message: errorMessage });
    }
  };
  ...
  return (
    <div className="market-header__sheet">
      ...
      <div className="market-header__sheet-content">
        ...
        <div className="market-header__panel">
          ...
          <div className="market-header__panel">
            <div className="market-header__row">
              <div className="market-header__row-main">
                <div className="settings-modal__section-title">{t('settings.theme')}</div>
              </div>
            </div>
                <div className="settings-modal__button-group">
                  <button
          ...
          <div className="market-header__panel">
            <div className="market-header__row">
              <div className="market-header__row-main">
                <div className="settings-modal__section-title">{t('settings.notificationsOffers')}</div>
              </div>
            </div>
            <div className="settings-modal__button-group">
              <button 
                ...
                onClick={async () => {
                  try {
                    ...
                    await updateSettingMutation.mutateAsync({ setting: 'offers_notifications', status: true });
                    showSuccessToast({ message: t('settings.notificationsEnabled') });
                  } catch (error) {
                    console.error('Failed to enable notifications:', error);
                    const errorMessage = (error as any)?.message || t('settings.notificationsUpdateFailed');
                    showErrorToast({ message: errorMessage });
                  }
                }}
              >
              ...
              <button 
                ...
                onClick={async () => {
                  try {
                    ...
                    await updateSettingMutation.mutateAsync({ setting: 'offers_notifications', status: false });
                    showSuccessToast({ message: t('settings.notificationsDisabled') });
                  } catch (error) {
      ...
      <div className="market-header__sheet-footer">
        <button className="market-header__btn-secondary" onClick={() => openLink(config.settings.politicsUrl)}>
          ...
          <span>{t('settings.politics')}</span>
        </button>
        <button className="market-header__btn-primary" onClick={() => openLink(config.settings.supportUrl)}>
          <svg width="24" height="23" viewBox="0 0 24 23" fill="none" xmlns="http://www.w3.org/2000/svg">
